...
class SignLanguage(Image):
    def __init__(self, capture, fps, **kwargs):
        super(SignLanguage, self).__init__(**kwargs)
...
class CamApp(App):
    def build(self):
        self.capture = cap
        self.capture = cv2.VideoCapture(detect(cap))
        self.my_camera = SignLanguage(capture=self.capture, fps=30)
